...
/*
    ...
    {
        $id = $this->bean->id;
        // get a text versio  of binary uuid
        if(self::toolbox()->getWriter() instanceof \RedBean_QueryWriter_MySQLUUID)
        {
            $id = bin2hex($id);
        }
        return $id;
    ...
    {
        if(strlen($id) == 16 && self::toolbox()->getWriter() instanceof \RedBean_QueryWriter_MySQLUUID)
        {
            $id = pack("h*", str_replace('-', '', $id));
        }
        $this->bean->id = $id;
    }
*/
